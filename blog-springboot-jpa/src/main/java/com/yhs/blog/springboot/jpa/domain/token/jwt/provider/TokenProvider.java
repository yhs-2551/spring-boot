...
public class TokenProvider {
    ...
    private String makeToken(Date expiry, User user) {
        ...
        return Jwts.builder()
                .issuer(jwtProperties.getIssuer()).issuedAt(now).expiration(expiry).subject(String.valueOf(user.getId()))
                .claim("id", user.getId())
    ...
    public boolean validateRefreshToken(String token, Long userId) {
        String storedRefreshToken = redisTemplate.opsForValue().get(TokenManagementService.RT_PREFIX + userId);
    ...
    private Claims getClaims(String token) {
        try {
            return Jwts.parser().verifyWith(jwtProperties.getJwtSecretKey()).build().parseSignedClaims(token)
                    .getPayload();
