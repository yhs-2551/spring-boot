...
- 여러 블로그 플랫폼(티스토리, 네이버 블로그, 워드프레스, 벨로그 등)을 사용하면서 느꼈던 불편했던점을 개선하고, 원하는 기능을 추가하기 위한 블로그 프로젝트
- 사용자별 처리, 게시물 관리, 검색 기능, 카테고리 시스템, 사용자별 프로필 업데이트, 이미지/파일 업로드(다중 이미지 포함)등 블로그 플랫폼에 필요한 핵심 기능 구현
...
*   **성능 최적화**
    ...
    *   **JPA 최적화**
        *   초기 연관관계 매핑 시 추상적인 코드로 인한 코드의 명확성 감소 및 불필요한 쿼리 증가로 성능이 저하될 가능성이 있음
